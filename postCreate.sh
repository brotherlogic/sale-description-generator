...
if [ -z "$TMUX" ] && [ -n "$PS1" ]; then
  cd /workspaces/sale-description-generator
  /workspaces/sale-description-generator/start-tmux.sh && tmux attach-session -t sale-description-generator
fi
EOF
)
...
/workspaces/sale-description-generator/start-tmux.sh
